			...
			<div class="row p-5">
				<?php 
					...
					if(!empty($textoBusqueda)){
						...
						$consulta = mysqli_query($conexion, "SELECT * FROM producto WHERE DESCRIPCION like '%" . $textoBusqueda . "%'") or die("No se puede realizar la consulta de búsqueda");
						$nFilas = mysqli_num_rows($consulta);

						if($nFilas > 0){
							for($i = 0; $i < $nFilas; $i++){
								$fila = mysqli_fetch_array($consulta);
								$consultaUsuario = mysqli_query($conexion, "SELECT NOMBRE FROM cliente WHERE ID_CLIENTE ='" . $fila['ID_CLIENTE'] . "'") or die("Error buscando el usuario");
									...
									<div class='col-12'>
										<div class='card'>
											...
											<div class='card-body bg-primary cuerpoCard'>
												...
												<span class='float-right'>Subido por: " . $filaUsuario['NOMBRE'] . "</span>
											</div>
										</div>
									</div>
									";
							...
							mysqli_close($conexion);
							echo "<div class='bg-white w-100 border p-4 cajaVacia'>
									<span class='alert'>Sin resultados para </span>
									<br/>
									<span class='alert font-weight-bold'>" . $textoBusqueda . "</span>
								</div>";
						}
					}else{
						echo "<div class='bg-white w-100 border p-4 cajaVacia'>
