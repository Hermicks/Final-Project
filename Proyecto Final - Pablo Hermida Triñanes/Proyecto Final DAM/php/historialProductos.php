...
<head>
	...
	<script type="text/javascript">
		function seleccionarIndice(){
				...
				location.href = "historialProductos.php?categoria=" + seleccion;
		}
	</script>
</head>
<body>
			...
			<div class="row p-2">
					<?php
						...
						if(!empty($idUsuario)){
							...
							$consulta = mysqli_query($conexion, "SELECT * FROM producto WHERE ID_CLIENTE='" . $idUsuario . "'") or die("Error en la consulta");
							...
							if($nFilas > 0){
											...
											<div class='col-12'>
												<div class='card'>
													...
													<div class='card-body bg-primary cuerpoCard'>
														<h3 class='card-title'>" . $fila['DESCRIPCION'] . "</h3>
								...
								echo "<script>alert('No has subido ningún producto');</script>";
								echo "<div style='height:250px;'></div>";
								mysqli_close($conexion);
							}

						}else{
							echo "<script>alert('No existe ningún usuario');</script>";
						}
					?>
			</div>
		</div>
