...
<body>
	<?php
		...
		if(isset($_POST['borraCuenta'])){
			$idBorrar = $_SESSION['id_Usuario'];
			...
			if(!empty($_POST['razon'])){
				...
				$deleteDeseos = mysqli_query($conexion, "DELETE FROM deseos WHERE ID_CLIENTE='" . $idBorrar . "'") or die("Error en el borrado de deseos");

				$deleteProducto = mysqli_query($conexion, "DELETE FROM producto WHERE ID_CLIENTE='" . $idBorrar . "'") or die("Error en el borrado de producto");

				$deleteCliente = mysqli_query($conexion, "DELETE FROM cliente WHERE ID_CLIENTE='" . $idBorrar . "'") or die("Error en el borrado de cliente");

				if($deleteDeseos && $deleteProducto && $deleteCliente){
				...
				echo "<script>alert('Por favor selecciona una opción antes de borrar tu cuenta');</script>";
			}
		}
	?>
	<header>
			...
			<div class="row justify-content-center">
				...
				<div class="col-10 m-5 p-3" id="formuStyle">
					...
					<form action="" method="post">
						<span class="font-weight-bold">¿Te importaría decirnos por que te vas para así poder mejorar?</span>
						<br/>
						<br/>
						<div class="radio"><label class="radio-inline"><input type="radio" name="razon">&nbspNo entiendo la aplicación</label></div>
						<div class="radio"><label class="radio-inline"><input type="radio" name="razon">&nbspNo me gusta la aplicación</label></div>
						<div class="radio"><label class="radio-inline"><input type="radio" name="razon">&nbspLa oferta es escasa</label></div>
						<div class="radio"><label class="radio-inline"><input type="radio" name="razon">&nbspHay sitios mejores</label></div>
						<div class="radio"><label class="radio-inline"><input type="radio" name="razon">&nbspPorque sí</label></div>
						<br/>
						<input type="submit" name="borraCuenta" value="Borra tu Cuenta NxtTech" class="btn btn-success w-100">
					</form>
				</div>
			<div>
		</div>
